...
**Example:** `https://app.getspot.org/join/ABC-DEF-GHI`
...
1. User taps link → Opens web page at `https://app.getspot.org/join/{CODE}`
2. Web page shows:
...
```
https://app.getspot.org/.well-known/apple-app-site-association
```
...
```
https://app.getspot.org/.well-known/assetlinks.json
```

## Important: Flutter Web Deployment

**Your Flutter app IS the website!** Since you're deploying the Flutter web build to `app.getspot.org`, the app will automatically handle `/join/{code}` routes via `onGenerateRoute` in `main.dart`.

**What this means:**
- ✅ When users visit `https://app.getspot.org/join/ABC-DEF-GHI` in a browser, they see your Flutter web app with the JoinGroupScreen
...
- The app is served exclusively from `app.getspot.org`; `www.getspot.org`/`getspot.org` now host a separate marketing site (see `main/` and `docs/hosting/README.md`), which redirects old `/join/{code}` links to `app.getspot.org`.

## Hosting Requirements

### Required Files on app.getspot.org

You only need to upload 2 files (not 3!) since Flutter web handles the landing page:

#### 1. Apple App Site Association

**Location:** `https://app.getspot.org/.well-known/apple-app-site-association`
...
**Location:** `https://app.getspot.org/.well-known/assetlinks.json`
...
~~**Location:** `https://app.getspot.org/join/index.html`~~
...
**Flow:**
1. User taps link: `https://app.getspot.org/join/ABC-DEF-GHI`
...
xcrun simctl openurl booted "https://app.getspot.org/join/ABC-DEF-GHI"
```
...
adb shell am start -W -a android.intent.action.VIEW \
  -d "https://app.getspot.org/join/ABC-DEF-GHI" com.getspot.app
```
...
4. Check verification status:
   ```bash
   adb shell pm get-app-links com.getspot.app
   # Should show: verified for domain app.getspot.org
   ```
...
- Example: `https://app.getspot.org/j/ABC` → redirects to `/join/ABC-DEF-GHI`
